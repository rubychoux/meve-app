...
import { cleanJson, fetchOpenAIWithTimeout } from '../../utils/openai';
...
interface TodaysLookData {
  ...
  generatedAt?: string;
  // MEVE-247 — present only when vibe-mode look conflicts with personal color
  conflictNote?: string | null;
}

// MEVE-247 — two distinct look generation modes.
type LookMode = 'personal_color' | 'vibe';

// ─── Helpers ───────────────────────────────────────────────────────────────

function todayKey(mode: LookMode) {
  const d = new Date();
  const datePart = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
  const suffix = mode === 'personal_color' ? 'personalcolor' : 'vibe';
  return `${TODAYS_LOOK_KEY_PREFIX}${suffix}_${datePart}`;
}
...
export function LookScreen() {
  ...
  // Today's look
  // MEVE-247 — two looks per day, cached separately by mode.
  const [lookMode, setLookMode] = useState<LookMode>('personal_color');
  const [looksByMode, setLooksByMode] = useState<Record<LookMode, TodaysLookData | null>>({
    personal_color: null,
    vibe: null,
  });
  const todaysLook = looksByMode[lookMode];
  ...
  // ── Today's look ─────────────────────────────────────────────────────────
  // MEVE-247 — mode-specific prompt builder.
  const buildLookPrompt = useCallback(
    (mode: LookMode, daysLeft: number | null): string => {
      if (mode === 'personal_color') {
        return `You are a Korean beauty expert recommending a makeup look.

IMPORTANT CONCEPT SEPARATION:
- Personal color = the color tone that naturally suits the user (determined by AI analysis)
- Vibe/style = the aesthetic the user WANTS (their personal preference, may differ from personal color)

Generate a makeup look based on the user's PERSONAL COLOR (natural color harmony):
Personal color: ${personalColor ?? '미분석'}
Event: ${eventType ?? '없음'}${daysLeft != null ? ` D-${daysLeft}` : ''}
Skin type: 미설정
Face shape: ${faceShape ?? '미분석'}

The look should maximize their natural color harmony.
Even if their vibe preference is different, focus on what suits their personal color.
Include a note like "퍼스널컬러에 자연스럽게 어울리는 룩이에요"

Return ONLY valid JSON (no markdown):
{
  "lookTitle": "룩 이름 한국어 20자 이내 (예: 여름 쿨톤 데일리 미스트)",
  "lookDescription": "이 룩이 퍼스널컬러에 어울리는 이유 2줄 (해요체)",
  "keyColors": ["#hex1", "#hex2", "#hex3"],
  "unsplashQuery": "english search query for this look type (e.g. 'cool tone winter makeup korean')",
  "steps": [
...
Generate a makeup look based on the user's PREFERRED VIBE/STYLE (what they want):
Preferred vibe: ${vibe ?? '글로우'}
Personal color (for reference only): ${personalColor ?? '미분석'}
Event: ${eventType ?? '없음'}${daysLeft != null ? ` D-${daysLeft}` : ''}

The look should match what the user WANTS, not just what suits them by color analysis.
It's completely valid to go outside personal color boundaries — this is about self-expression.
Include a note like "원하는 추구미를 살린 룩이에요"

If personal color and vibe conflict (예: 겨울 쿨톤 + 봄 웜톤 추구미):
- Acknowledge it naturally: "퍼스널컬러와 다를 수 있지만, 원하는 스타일을 살렸어요"
- Provide bridging tips: how to make it work for their coloring

Return ONLY valid JSON (no markdown):
{
  "lookTitle": "룩 이름 한국어 20자 이내",
  "lookDescription": "이 룩이 추구미를 어떻게 표현하는지 2줄 (해요체)",
  "keyColors": ["#hex1", "#hex2", "#hex3"],
  "unsplashQuery": "english search query for this vibe (e.g. 'warm glow dewy makeup korean')",
  "steps": [
    {"category": "베이스", "tip": "1줄 팁 해요체"},
    {"category": "아이", "tip": "1줄 팁 해요체"},
    {"category": "립", "tip": "1줄 팁 해요체"}
  ],
  "conflictNote": "퍼스널컬러와 추구미가 다를 때만 1줄 (해요체) 보강 팁. 다르지 않으면 null"
}`;
    },
    [personalColor, vibe, eventType, faceShape]
  );

  const generateTodaysLook = useCallback(
    async (mode: LookMode) => {
      ...
      try {
        ...
        const prompt = buildLookPrompt(mode, daysLeft);
        ...
        const queryFallback =
          mode === 'personal_color'
            ? `${personalColor ?? 'natural'} korean makeup`
        ...
        setLooksByMode((prev) => ({ ...prev, [mode]: finalLook }));
        try {
          await AsyncStorage.setItem(key, JSON.stringify(finalLook));
        } catch {}
      } catch {
        setLooksByMode((prev) => ({ ...prev, [mode]: null }));
      } finally {
        setLoadingLook(false);
      }
    },
    [buildLookPrompt, eventDate, eventType, personalColor, vibe]
  );

  const loadTodaysLook = useCallback(
    async (mode: LookMode) => {
      const key = todayKey(mode);
      try {
        ...
        if (cached) {
          setLooksByMode((prev) => ({
            ...prev,
            [mode]: JSON.parse(cached) as TodaysLookData,
          }));
          return;
        }
      } catch {}
      await generateTodaysLook(mode);
    },
    [generateTodaysLook]
  );

  // Lazy load: when user switches tab, load that mode's look if not yet loaded.
  useEffect(() => {
    if (looksByMode[lookMode] == null) {
      loadTodaysLook(lookMode);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lookMode]);

  // MEVE — when eventType changes, invalidate BOTH caches + regenerate current mode.
  ...
  useEffect(() => {
    ...
    AsyncStorage.multiRemove([
      todayKey('personal_color'),
      todayKey('vibe'),
    ]).catch(() => {});
    setLooksByMode({ personal_color: null, vibe: null });
    generateTodaysLook(lookMode);
  }, [eventType, generateTodaysLook, lookMode]);

  const onRefresh = async () => {
    setRefreshing(true);
    try {
      await AsyncStorage.removeItem(todayKey(lookMode));
    } catch {}
    await generateTodaysLook(lookMode);
    setRefreshing(false);
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        {/* MEVE-247 — 두 가지 룩 모드 탭 */}
        <View style={styles.lookModeTabRow}>
          <TouchableOpacity
            style={[
              styles.lookModeTab,
              lookMode === 'personal_color' && styles.lookModeTabActive,
            ]}
            onPress={() => setLookMode('personal_color')}
            activeOpacity={0.85}
          >
            <Text
              style={[
                styles.lookModeTabText,
                lookMode === 'personal_color' && styles.lookModeTabTextActive,
              ]}
            >
              💙 퍼스널컬러 룩
            </Text>
          ...
          <TouchableOpacity
            style={[
              styles.lookModeTab,
              lookMode === 'vibe' && styles.lookModeTabActive,
            ]}
            onPress={() => setLookMode('vibe')}
            activeOpacity={0.85}
          >
            <Text
              style={[
                styles.lookModeTabText,
                lookMode === 'vibe' && styles.lookModeTabTextActive,
              ]}
            >
              ✨ 추구미 룩
            </Text>
        ...
        <Text style={styles.lookModeExplain}>
          {lookMode === 'personal_color'
            ? '💙 타고난 색감에 자연스럽게 어울리는 룩'
            : '✨ 내가 원하는 스타일로 표현하는 룩'}
        </Text>
...
function TodaysLookHero({
  ...
  return (
    <View style={styles.heroSection}>
      ...
      {/* MEVE-247 — vibe-mode conflict bridging note */}
      {look?.conflictNote && (
        <View style={styles.conflictNote}>
          <Text style={styles.conflictNoteText}>💡 {look.conflictNote}</Text>
        </View>
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  // MEVE-247 — Look mode tabs + explain + conflict note
  lookModeTabRow: {
    flexDirection: 'row',
    marginHorizontal: 20,
    marginBottom: 12,
  ...
  dashedTitle: { fontSize: 14, fontWeight: '700', color: '#1A1A1F' },
  dashedDesc: {
    fontSize: 13,
    color: '#8A8A9A',
    textAlign: 'center',
  ...
  colorMatchTitle: { fontSize: 14, fontWeight: '700', color: '#1A1A1F' },
  ...
  inspoTitle: { fontSize: 15, fontWeight: '800', color: '#1A1A1F' },
  inspoDesc: { fontSize: 13, color: '#5A5A65', lineHeight: 19 },

  // Section 4 — Personal color
  pcType: { fontSize: 22, fontWeight: '800', color: '#1A1A1F' },
  ...
  faceIntroTitle: { fontSize: 14, fontWeight: '700', color: '#1A1A1F' },
  ...
  solutionTitle: { fontSize: 15, fontWeight: '700', color: '#1A1A1F' },
